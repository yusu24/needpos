...
export default function Index({ supplierReturns, filters }) {
  ...
  const handleConfirm = (id) => {
    if (confirm('Apakah Anda yakin ingin mengkonfirmasi retur ini? Stok barang akan dikurangi secara otomatis.')) {
      router.post(route('admin.supplier-returns.confirm', id));
    }
  };
  ...
  return (
    <AppLayout header="Retur Ke Supplier">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col md:flex-row gap-3 justify-between items-start md:items-center bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm">
          <div>
            <h2 className="text-sm font-bold text-slate-800">Daftar Retur Pemasok</h2>
            <p className="text-xs text-slate-500 font-medium">Pengembalian barang rusak / overstock ke supplier pemasok.</p>
          </div>
          ...
          <div className="flex w-full md:w-auto items-center gap-3">
            ...
            <Link
              href={route('admin.supplier-returns.create')}
                ...
                {supplierReturns.data.length > 0 ? (
                  supplierReturns.data.map((sr) => (
                    <tr key={sr.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-slate-600">{sr.reason || '-'}</td>
                      <td className="py-3.5 px-4 font-bold text-slate-900">{formatCurrency(sr.total_amount)}</td>
                      <td className="py-3.5 px-4 text-center">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                          ...
                          {sr.status === 'confirmed' ? 'Selesai' : 'Draft'}
                        </span>
                      </td>
